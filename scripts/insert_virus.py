...
def main(argv):
    ...
    parser.add_argument(
        "--ints", help="output fasta file", type=str, default="integrations.fa"
    )
    parser.add_argument(
        "--int_info",
        help="output tsv with info about viral integrations",
        type=str,
        default="integrations.tsv",
    )
    parser.add_argument(
        "--int_num",
        help=f"number of integrations to be carried out [{default_ints}]",
        type=int,
        default=default_ints,
    )
    parser.add_argument(
        "--p_whole",
        help=f"probability of a virus being integrated completely [{default_p_whole}]",
        type=float,
        default=default_p_whole,
    )
    parser.add_argument(
        "--p_rearrange",
        help=f"probability of an integrated piece of virus being rearranged [{default_p_rearrange}]",
        type=float,
        default=default_p_rearrange,
    )
    parser.add_argument(
        "--p_delete",
        help=f"probability of an integrated piece of virus containing a deletion [{default_p_delete}]",
        type=float,
        default=default_p_delete,
    )
    parser.add_argument(
        "--lambda_split",
        help=f"mean of poisson distriubtion for number of pieces to split an integrated viral chunk into when rearranging or deleting [{default_lambda_split}]",
        type=float,
        default=default_lambda_split,
    )
    parser.add_argument(
        "--p_overlap",
        help=f"probability of a junction to be overlapped (common bases between virus and host) [{default_p_overlap}]",
        type=float,
        default=default_p_overlap,
    )
    parser.add_argument(
        "--p_gap",
        help=f"probability of a junction to have a gap [{default_p_gap}]",
        type=float,
        default=default_p_gap,
    )
    parser.add_argument(
        "--lambda_junction",
        help=f"mean of poisson distribution of number of bases in a gap/overlap [{default_lambda_junction}]",
        type=float,
        default=default_lambda_junction,
    )
    parser.add_argument(
        "--p_host_deletion",
        help=f"probability of a host deletion at the integation site [{default_p_host_del}]",
        type=float,
        default=default_p_host_del,
    )
    parser.add_argument(
        "--lambda_host_deletion",
        help=f"mean of poisson distribution of number of bases deleted from the host [{default_lambda_host_del}]",
        type=float,
        default=default_lambda_host_del,
    )
    parser.add_argument(
        "--epi_num",
        help=f"number of episomes to added to output fasta [{default_ints}]",
        type=int,
        default=default_ints,
    )
    parser.add_argument(
        "--epi_info",
        help="output tsv with info about episomes",
        type=str,
        default="episomes.tsv",
    )
    parser.add_argument(
        "--seed", help="seed for random number generator", default=12345, type=int
    )
    parser.add_argument(
        "--verbose", help="display extra output for debugging", action="store_true"
    )
    parser.add_argument(
        "--model-check",
        help="check integration model every new integration",
        action="store_true",
    )
    parser.add_argument(
        "--min-sep",
        help="minimum separation for integrations",
        type=int,
        default=default_min_sep,
    )
    parser.add_argument(
        "--min-len",
        help="minimum length of integrated viral fragments",
        type=int,
        default=None,
    )
    parser.add_argument(
        "--max-len",
        help="maximum length of integrated viral fragments",
        type=int,
        default=None,
    )
...
class Events(dict):
    ...
    def __init__(
        ...
        min_len=None,
        max_len=None,
        simug_snp_indel=None,
        simug_cnv=None,
        simug_inversion=None,
        simug_tranlocation=None,
    ):
        """
        ...
        if self.checkFastaExists(host_fasta_path):
            self.host = SeqIO.to_dict(
                SeqIO.parse(host_fasta_path, "fasta", alphabet=unambiguous_dna)
            )
        else:
        ...
        if self.checkFastaExists(virus_fasta_path):
            self.virus = SeqIO.to_dict(
                SeqIO.parse(virus_fasta_path, "fasta", alphabet=unambiguous_dna)
            )
        ...
        if simug_snp_indel is not None:
            self.indel = [
                row
                for row in self.read_simug_file(simug_snp_indel)
                if row["variant_type"] == "INDEL"
            ]
        else:
            self.indel = None
        ...
        if self.min_len is not None:
            if not all([len(virus) >= self.min_len for virus in self.virus.values()]):
                raise ValueError(
                    f"specified minimum length is more than the length of one of the input viruses"
                )
            if any([len(virus) == self.min_len for virus in self.virus.values()]):
                print(
                    f"warning: minimum length is equal to the length of one or more references - integrations involving these references will all be whole, regardless of p_whole"
                )
        ...
        if self.max_len is not None:
            min_virus_len = min([len(virus) for virus in self.virus.values()])
            if self.max_len > min_virus_len:
                print(
                    f"overridding maximum length {self.max_len} to {min_virus_len} as it is longer than the shortest viral reference"
                )
                self.max_len = min_virus_len
    ...
    def add_integrations(
        self, probs, int_num, max_attempts=50, model_check=False, min_sep=1
    ):
        """
        ...
        if "ints" in self:
            raise ValueError(
                "integrations have already been added to this Events object"
            )  # is there a better error type for this?
        ...
        if self.min_sep * 4 * int_num > total_host_length:
            raise ValueError(
                "The requested number of integrations, with the specified minimum separation, are not likely to fit into the specified host.  Either decrease the number of integrations or the minimum separation."
            )
...
class Integrations(list):
    ...
    def __init__(
        ...
        self.check_prob(
            self.probs["p_overlap"] + self.probs["p_gap"],
            "the sum of p_overlap and p_gap",
        )
        ...
        self.check_float_or_int_positive(
            self.probs["lambda_junction"], "lambda_junction"
        )
        self.check_float_or_int_positive(
            self.probs["lambda_host_del"], "lambda_host_deletion"
        )
        ...
        self.model = {
            chr: [
                {"origin": "host", "coords": (0, len(seq)), "ori": "+", "seq_name": chr}
            ]
            for chr, seq in host.items()
        }
    ...
    def check_float_or_int_positive(self, value, name):
        ...
        if value <= 0:
            raise ValueError(
                f"{name} must be greater than zero (it's currently {value})"
            )
    ...
    def poisson_with_minimum_and_maximum(self, lambda_poisson, min=-np.inf, max=np.inf):
        ...
        while True:
            ...
            if counter > self.max_attempts:
                print(
                    f"warning: too many attempts to get value with minimum {min} and maximum {max} from poisson distribution with mean {lambda_poisson}",
                    flush=True,
                )
    ...
    def set_junc_properties(self):
        ...
        junc_props["junc_types"] = self.rng.choice(
            a=["overlap", "gap", "clean"], size=2, p=prob_juncs
        )
        ...
        if junc_props["junc_types"][0] == "clean":
            ...
            n_left_junc = self.poisson_with_minimum_and_maximum(
                self.probs["lambda_junction"], min=1
            )
        ...
        if junc_props["junc_types"][1] == "clean":
            ...
            n_right_junc = self.poisson_with_minimum_and_maximum(
                self.probs["lambda_junction"], min=1
            )
        ...
        assert (
            not (junc_props["junc_types"][0] == "clean") or junc_props["n_junc"][0] == 0
        )
        assert (
            not (junc_props["junc_types"][1] == "clean") or junc_props["n_junc"][1] == 0
        )
        ...
        host_deletion = self.rng.choice(
            a=[True, False], p=(self.probs["p_host_del"], 1 - self.probs["p_host_del"])
        )
        ...
        if junc_props["host_del"] is True:
            junc_props["host_del_len"] = self.poisson_with_minimum_and_maximum(
                self.probs["lambda_host_del"], min=1
            )
    ...
    def set_chunk_properties(self):
        ...
        chunk_props["is_whole"] = bool(
            self.rng.choice(a=[True, False], p=[self.probs["p_whole"], p_portion])
        )
        ...
        is_rearrange = bool(
            self.rng.choice(
                a=[True, False], p=[self.probs["p_rearrange"], p_not_rearrange]
            )
        )
        ...
        is_delete = bool(
            self.rng.choice(a=[True, False], p=[self.probs["p_delete"], p_not_delete])
        )
        ...
        if is_delete is False and is_rearrange is False:
            ...
            chunk_props["n_fragments"] = self.poisson_with_minimum_and_maximum(
                self.probs["lambda_split"], min=min_split
            )
        ...
        if is_delete is True:
            chunk_props["n_delete"] = int(
                self.rng.choice(range(0, chunk_props["n_fragments"] - 1))
            )
        else:
        ...
        if is_rearrange is True:
            chunk_props["n_swaps"] = self.poisson_with_minimum_and_maximum(
                self.probs["lambda_split"], min=1
            )
    ...
    def __update_model(self, integration):
        ...
        for i, seg in enumerate(self.model[integration.chr]):
            if seg["origin"] != "host":
                continue
            if (
                integration.pos >= seg["coords"][0]
                and integration.pos <= seg["coords"][1]
            ):
                break
        ...
        left_overlap_bases = (
            integration.junc_props["n_junc"][0]
            if integration.junc_props["junc_types"][0] == "overlap"
            else 0
        )
        right_overlap_bases = (
            integration.junc_props["n_junc"][1]
            if integration.junc_props["junc_types"][1] == "overlap"
            else 0
        )
        ...
        if integration.junc_props["junc_types"][0] in ["gap", "overlap"]:
            ...
            # overlap features
                ...
                ambig["coords"] = (
                    integration.pos,
                    integration.pos + left_overlap_bases,
                )
                assert (
                    str(
                        self.host[integration.chr][
                            ambig["coords"][0] : ambig["coords"][1]
                        ].seq
                    ).lower()
                    == integration.junc_props["junc_bases"][0].lower()
                )
            else:
                raise ValueError(
                    f"unrecgonised integration type: {integration.junc_props[0]}"
                )
        ...
        for j in range(len(integration.chunk.pieces)):
            virus = {
                "origin": "virus",
                "coords": (
                    integration.chunk.pieces[j][0],
                    integration.chunk.pieces[j][1],
                ),
        ...
        assert (
            len(integration.junc_props["junc_bases"][1])
            == integration.junc_props["n_junc"][1]
        )
        if integration.junc_props["junc_types"][1] in ["gap", "overlap"]:
            ...
            # overlap features
                ...
                assert (
                    ...
                    == integration.junc_props["junc_bases"][1].lower()
                )
            else:
                raise ValueError(
                    f"unrecgonised integration type: {integration.junc_props[0]}"
        ...
        host["coords"] = (
            integration.pos + integration.junc_props["host_del_len"] + overlap_bases,
            host_stop,
        )
            ...
            for seg in self.model[chr]:
                ...
                if seg["origin"] == "host":
                    ...
                    if seg["seq_name"] != "gap":
                        ...
                        host_bases = str(
                            self.host[chr][seg["coords"][0] : seg["coords"][1]].seq
                        ).lower()
                ...
                for entry in self.model[chr]:
                    ...
                    # if viral
                        ...
                        if entry["ori"] == "+":
                            ...
                            raise ValueError(
                                f"unregconised orientation {entry['ori']} in {entry}"
                            )

                    else:
                        raise ValueError(
                            f"unrecgonised model feature on chr {chr}: {entry}"
                        )
    ...
    def __handle_indels(self, integration):
        ...
        while i < len(self.indel_list_for_consuming):
            ...
            if integration.pos >= int(indel["sim_end"]):
                host_position_offset += len(indel["sim_allele"]) - len(
                    indel["ref_allele"]
                )
        ...
        while i < len(self.indel_list_for_consuming):
            ...
            if (
                int(indel["sim_start"]) <= integration.pos
                and int(indel["sim_end"]) > integration.pos
            ):
        ...
        integration.host_deleted_original_fasta = (
            integration.host_deleted + host_deletion_offset
        )
    ...
    def __save_info(self, filename):
        ...
        previous_ints = {
            key: 0 for key in self.host.keys()
        }  # length of previous integrations
        deleted_bases_input_fasta = {
            key: 0 for key in self.host.keys()
        }  # deletions because of integrations
        deleted_bases_original_fasta = {key: 0 for key in self.host.keys()}
        sim_to_original_offset = {
            key: 0 for key in self.host.keys()
        }  # add this to sim position to get original position (due to indels)
        ...
        with open(filename, "w", newline="") as csvfile:
            ...
            for i, integration in enumerate(self):
                ...
                host_position_offset, host_deletion_offset = self.__handle_indels(
                    integration
                )
                ...
                left_start = (
                    integration.pos
                    + previous_ints[integration.chr]
                    - deleted_bases_input_fasta[integration.chr]
                )
                ...
                if integration.junc_props["junc_types"][0] == "gap":
                    previous_ints[integration.chr] += integration.junc_props["n_junc"][
                        0
                    ]
                if integration.junc_props["junc_types"][1] == "gap":
                    previous_ints[integration.chr] += integration.junc_props["n_junc"][
                ...
                deleted_bases_input_fasta[integration.chr] += integration.junc_props[
                    "host_del_len"
                ]
                deleted_bases_original_fasta[
                    integration.chr
                ] += integration.host_deleted_original_fasta
                ...
                junc_lengths = ",".join(
                    [str(i) for i in integration.junc_props["n_junc"]]
                )
...
class Integration(dict):
    ...
    def __init__(
        self, host, virus, model, rng, int_id, chunk_props, junc_props, min_sep
    ):
        """
        ...
        assert all(
            [i in ["clean", "gap", "overlap"] for i in self.junc_props["junc_types"]]
        )
        ...
        if self.chunk_props["max_len"] is not None:
            if self.chunk_props["max_len"] < self.chunk_props["min_len"]:
                raise ValueError(
                    'The maximum length ({self.chunk_props["max_len"]}) is less than the length of the overlapped bases ({self.n_overlap_bases()}).  Please specify a longer maximum length or a smaller number for the mean of the number of bases involved in the junction.'
                )

        # get viral chunk
        self.chunk = ViralChunk(virus, rng, self.chunk_props)
        ...
        self.junc_props["junc_bases"] = (
            self.get_junc_bases(rng, "left"),
            self.get_junc_bases(rng, "right"),
        )
        ...
        if junc_props["host_del"] is False:
            ...
            for seg in model[self.chr]:
                ...
                if not self.has_overlap(
                    self.pos, self.pos, seg["coords"][0], seg["coords"][1]
                ):
                    continue

                # are we trying to delete past the end of the segment?
                if self.pos + self.host_deleted + self.n_overlap_bases() >= (
                    seg["coords"][1] - min_sep
                ):
                    self.host_deleted = (
                        seg["coords"][1] - self.pos - min_sep - self.n_overlap_bases()
                    )
    ...
    def get_random_position(self, model, rng, min_sep):
        ...
        host_coords = [
            segment["coords"]
            for segment in model[self.chr]
            if segment["origin"] == "host"
        ]

        # enforce minimum separation
        host_coords = [
            (coords[0] + min_sep, coords[1] - min_sep) for coords in host_coords
        ]
    ...
    def get_int_position(self, chr, rng, model, min_sep):
        ...
        if all(
            [
                True if (i in ["clean", "gap"]) else False
                for i in self.junc_props["junc_types"]
            ]
        ):
        ...
        # if overlap at both ends, look for both overlaps in host chromosome next to each other
            ...
            self.junc_props["junc_types"][0] == "overlap"
            and self.junc_props["junc_types"][1] == "overlap"
        ):
            # make string with both overlap bases
            self.pos = self.find_overlap_bases(
                self.junc_props["junc_bases"][0] + self.junc_props["junc_bases"][1],
                chr,
                rng,
                model,
                min_sep,
        ...
        # left overlap
            ...
            self.pos = self.find_overlap_bases(
                self.junc_props["junc_bases"][0], chr, rng, model, min_sep
            )
            # check for unsuccessful find
            if self.pos == -1:
                self.pos = None
        ...
        # right overlap
            ...
            # find position with overlap at which to do overlap
            self.pos = self.find_overlap_bases(
                self.junc_props["junc_bases"][1], chr, rng, model, min_sep
            )
            # check for unsuccessful find
            if self.pos == -1:
                self.pos = None
            ...
            raise ValueError(
                f"junction types {self.junc_props['junc_types']} are not implemented yet"
            )
    ...
    def find_overlap_bases(self, overlap_bases, chr, rng, model, min_sep):
        ...
        for seg in model[self.chr]:
            ...
            if seg["ori"] == "+":
                found = re.search(
                    overlap_bases, str(chr[search_start:search_stop]), re.IGNORECASE
                )
                if found:
                ...
                raise ValueError(
                    "host chromosome segment {seg} has a negative orientation!"
                )
    ...
    def delete_left_bases(self, n):
        ...
        while deleted_bases < n:
            ...
            # because self.chunk.bases has already taken orientations into account
                ...
                print(
                    f"unrecgonised orientation {self.chunk.oris[i]} in chunk {vars(self.chunk)}"
                )
                self.pos = None
                return
        ...
        self.chunk.oris = [
            self.chunk.oris[i]
            for i in range(len(self.chunk.oris))
            if (i not in to_delete)
        ]
    ...
    def get_junc_bases(self, rng, side):
        ...
        if side == "left":
            ...
            # first n bases of viral chunk in an overlap
                ...
                raise ValueError(
                    f"unrecgonised type: {self.junc_props['junc_types'][0]}"
                )
        elif side == "right":
            ...
            if self.junc_props["junc_types"][1] == "clean":
                ...
                raise ValueError(
                    f"unrecgonised type: {self.junc_props['junc_types'][1]}"
                )
        else:
...
class ViralChunk(dict):
    ...
    def __init__(self, virus, rng, chunk_props):
        ...
        if self.chunk_props["is_whole"] is True:
            ...
            self.start = int(
                rng.integers(
                    low=0, high=len(virus[self.virus].seq) - chunk_props["min_len"]
                )
            )
            if chunk_props["max_len"] is None:
                self.stop = int(
                    rng.integers(
                        low=self.start + chunk_props["min_len"],
            ...
            raise ValueError(
                "self.chunk_props['is_whole'] must be either True or False"
            )
    ...
    def __split_into_pieces(self, rng):
        ...
        breakpoints = rng.choice(
            range(self.start + 1, self.stop - 1), size=num_breakpoints, replace=False
        )
    ...
    def __delete(self, rng):
        ...
        i_delete = rng.choice(
            range(1, len(self.pieces) - 1), self.chunk_props["n_delete"], replace=False
        )

        # do deletion
        self.pieces = [
            piece for i, piece in enumerate(self.pieces) if i not in i_delete
        ]
        self.oris = [ori for i, ori in enumerate(self.oris) if i not in i_delete]

        assert (
            len(self.pieces)
            == self.chunk_props["n_fragments"] - self.chunk_props["n_delete"]
        )
